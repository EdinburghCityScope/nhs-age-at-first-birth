...
const csvFilename = 'age-at-first-birth.csv';
...
edinburghcityscopeUtils.fetchGovBoundaries('dz-2001', (err, boundaries, zones2001) => {
    if (err) throw err;
    ...
    edinburghcityscopeUtils.fetchGovBoundaries('dz-2011', (err, boundaries, zones2011) => {
        if (err) throw err;
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?period ?zone ?age ?measure ?value
            WHERE {
                ?s qb:dataSet data:age-at-first-birth ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:age ?a ;
                   ldim:refArea ?z ;
